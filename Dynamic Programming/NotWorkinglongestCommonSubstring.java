...
class Solution{
    ...
    int longestCommonSubstr(String S1, String S2, int n, int m){
        ...
        return LCSubStrM2A2(S1.toCharArray(),S2.toCharArray(),n,m,0);
        //return res == 0 ?1:res;
    }

    int LCSubStrM2A2(char[] X, char[] Y, int m, int n, int lcsCount) {
		if (m <= 0 || n <= 0)
			return lcsCount;
		...
		if (X[m - 1] == Y[n - 1])
			return dp[m][n][lcsCount] = LCSubStrM2A2(X, Y, m - 1, n - 1, lcsCount + 1);
